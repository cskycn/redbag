...
class Html extends TagLib
{
    ...
    public function _editor($tag, $content)
    {
        ...
        switch (strtoupper($type)) {
            case 'UEDITOR':
                $parseStr   =  "\n".'<script type="text/javascript" charset="utf-8" src="__ROOT__/Public/statics/ueditor/ueditor.config.js"></script>'."\n".'<script type="text/javascript" charset="utf-8" src="__ROOT__/Public/statics/ueditor/ueditor.all.min.js"></script>'."\n".'<script type="text/plain" id="'.$id.'" name="'.$name.'" style="'.$style.'">'.$content.'</script>'."\n".'<script type="text/javascript">var ue_'.$id.' = UE.getEditor("'.$id.'");</script>'."\n";
                break;
            default:
